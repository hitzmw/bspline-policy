"""Controlled-ablation config tests for pure Drifting CoffeePressButton."""
...
def test_coffee_direct_drifting_has_dense_action_contract():
    config = _compose(
        "train_drifting_unet_coffee_press_button_image_workspace"
    )
    ...
    assert "bspline" not in config.policy._target_.split(".")[-2].lower()
    assert "bspline" not in config.task.dataset._target_.split(".")[-2].lower()


def test_coffee_direct_drifting_matches_bspline_training_settings():
    direct = _compose(
        "train_drifting_unet_coffee_press_button_image_workspace"
    )
    bspline = _compose(
        "train_drifting_unet_coffee_press_button_image_bspline_workspace"
    )
    ...
    assert direct.dataloader.batch_size == bspline.dataloader.batch_size == 32
    assert direct.val_dataloader.batch_size == 32
    assert bspline.val_dataloader.batch_size == 32
