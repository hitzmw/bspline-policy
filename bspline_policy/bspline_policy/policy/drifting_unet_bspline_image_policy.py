...
class DriftingUnetBSplineImagePolicy(BaseImagePolicy):
    ...
    ``[knot, control_point...]`` action representation.
    """
    ...
    def __init__(
        ...
        self.action_dim = self.regular_action_dim + 1
        bspline_shape_meta["action"]["shape"] = [self.action_dim]
    ...
    def compute_loss(
        ...
        if horizon != self.horizon or action_dim != self.action_dim:
            raise ValueError(
                "Expected normalized B-spline actions with shape "
