...
def make_empty_preprocessed_sample_cache(
    ...
    length: int = 0,
    extra_shapes_and_dtypes: Optional[dict] = None,
    ...
    extra_tensors = {}
    for key, (shape, dtype) in (extra_shapes_and_dtypes or {}).items():
        extra_tensors[key] = torch.empty(
            (length,) + tuple(shape), dtype=dtype, device=device
        )
    ...
    if share_memory:
        ...
        for tensor in extra_tensors.values():
            tensor.share_memory_()

    return {"obs": obs_tensors, "action": action_tensor, **extra_tensors}
...
def build_preprocessed_sample_cache(
    ...
    value_tensors = {
        key: torch.empty(
            (length,) + tuple(torch.as_tensor(value).shape),
            dtype=torch.as_tensor(value).dtype,
            device=cache_device,
        )
    ...
    if share_memory:
        for tensor in obs_tensors.values():
            tensor.share_memory_()
        for tensor in value_tensors.values():
            tensor.share_memory_()
    ...
    for idx in tqdm(range(length), desc=desc):
        ...
        for key, value in sample.items():
            if key != "obs":
                value_tensors[key][idx].copy_(
                    _to_tensor_on_device(value, cache_device)
                )

    return {"obs": obs_tensors, **value_tensors}
